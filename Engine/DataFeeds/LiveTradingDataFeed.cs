            ...
            if (timeTriggered != null)
            {
                ...
                if (timeTriggered is UserDefinedUniverse)
                {
                    ...
                    userDefined.CollectionChanged += (sender, args) =>
                    {
                        ...
                        if (items == null || _frontierTimeProvider.GetUtcNow() == DateTime.MinValue) return;

                        var symbol = items.OfType<Symbol>().FirstOrDefault();
                        if (symbol == null) return;

                        var collection = new BaseDataCollection(_frontierTimeProvider.GetUtcNow(), symbol);
                        var changes = _universeSelection.ApplyUniverseSelection(userDefined, _frontierTimeProvider.GetUtcNow(), collection);
